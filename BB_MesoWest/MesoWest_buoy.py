...
def get_buoy_ts(start_time, end_time, stationID='GSLBY'):
    ...
    try:
        air_temp = np.array(data['STATION'][0]["OBSERVATIONS"][
                            "air_temp_set_1"], dtype=float)
    except:
        air_temp = np.ones(len(DATES)) * np.nan
    try:
        Water_temp1 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_1"], dtype=float)
    except:
        Water_temp1 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp2 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_2"], dtype=float)
    except:
        Water_temp2 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp3 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_3"], dtype=float)
    except:
        Water_temp3 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp4 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_4"], dtype=float)
    except:
        Water_temp4 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp5 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_5"], dtype=float)
    except:
        Water_temp5 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp6 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_6"], dtype=float)
    except:
        Water_temp6 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp7 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_7"], dtype=float)
    except:
        Water_temp7 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp8 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_8"], dtype=float)
    except:
        Water_temp8 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp9 = np.array(data['STATION'][0]["OBSERVATIONS"][
                               "T_water_temp_set_9"], dtype=float)
    except:
        Water_temp9 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp10 = np.array(data['STATION'][0]["OBSERVATIONS"][
                                "T_water_temp_set_10"], dtype=float)
    except:
        Water_temp10 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp11 = np.array(data['STATION'][0]["OBSERVATIONS"][
                                "T_water_temp_set_11"], dtype=float)
    except:
        Water_temp11 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp12 = np.array(data['STATION'][0]["OBSERVATIONS"][
                                "T_water_temp_set_12"], dtype=float)
    except:
        Water_temp12 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp13 = np.array(data['STATION'][0]["OBSERVATIONS"][
                                "T_water_temp_set_13"], dtype=float)
    except:
        Water_temp13 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp14 = np.array(data['STATION'][0]["OBSERVATIONS"][
                                "T_water_temp_set_14"], dtype=float)
    except:
        Water_temp14 = np.ones(len(DATES)) * np.nan
    try:
        Water_temp15 = np.array(data['STATION'][0]["OBSERVATIONS"][
                                "T_water_temp_set_15"], dtype=float)
    except:
