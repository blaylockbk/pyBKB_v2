...
## st and et are the start and end times as strings: YYYYMMDDHH like '2014050618'
	print 'loading data...'
...
## ML_height is the array containing the Mixed Layer heights. If local time, need to input ML_height_loc
	...
	for j in range(phh.shape[1]):
		phh[:,j] = pheight[j]
	figname = outputDir + station+"_"+sd+"_"+str(int(bottom))+"-"+str(int(top))+"_"+type+"_ozone.png"
	...
	"""
######### Ozone Plot #########
	O_times, ozone2, ozone3 = load_ozone_data()
	ax = plt.subplot(2,1,2)
	plt.plot(O_times, ozone2, 'b', linewidth=2, label='Gusher')
	plt.plot(O_times, ozone3, 'r', linewidth=2, label='Lapoint')
