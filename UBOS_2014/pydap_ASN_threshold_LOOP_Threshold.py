...
troubleshooting:
	1) Check that the dates are correct. 
	2) For 2013 ROOSC data, use the example above, otherwise you
	   need to change the 'file_list' variable in the 'load_ROOS_data' function. 
	   The parameters for ROOSC are very sensitive. 
"""
...
def make_plots(type, bs_final, ML_height, time_restriction, thresh_str):
...
## ML_height is the array containing the Mixed Layer heights. If local time, need to input ML_height_loc
	...
	figname = outputDir + station+"_"+sd+"_"+str(int(bottom))+"-"+str(int(top))+"_"+type+"_T-"+thresh_str+".png"
...
for threshold in np.arange(-8.,-6.9,.1):

	thresh_str = str(int(abs(threshold*10)))
	print thresh_str
	print threshold
	...
	if plot_utc:
		...
		make_plots('UTC', bs_final, ML_height, time_restriction, thresh_str)

	if plot_local:

		make_plots('LOCAL', bs_final_loc, ML_height_loc, time_restriction_loc,thresh_str)
