...
# Brackground image
m.arcgisimage(service='NatGeo_World_Map', xpixels = 2500, dpi=100, verbose= True)
###############################################################################
...
for info, shape in zip(m.perim_info, m.perim):
    ...
    if info['FIRENAME']== 'PIONEER' and info['PERDATTIME']==[2016,8,6] and np.shape(shape)[0]>6000 and info['SHAPENUM']==1914:
        ...
        print ""
ax.add_collection(PatchCollection(patches, facecolor= 'indianred', alpha=.75, edgecolor='k', linewidths=1.75, zorder=1))
...
for info, shape in zip(m.perim_info, m.perim):
    ...
    if info['FIRENAME']== 'PIONEER' and info['PERDATTIME']==[2016,8,4] and np.shape(shape)[0]>6000 and info['SHAPENUM']==1860:
        ...
        print ""
ax.add_collection(PatchCollection(patches2, facecolor= 'maroon', alpha=.65, edgecolor='k', linewidths=1.5, zorder=1))
...
m.scatter(lons,lats,color='darkorange',s=150,zorder=500)
