...
for this in range(0,len(dirs)):
    SAVEDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/Pioneer_Fire/UpDownDrafts/'+dirs[this]+'/'
    ...
    for analysis_h in ah:
        ...
        # since the name for this field is "unkown" we have to get it by the file's row number
        grbs.seek(0) #start from top line
        grbs.seek(30)# go to line 30
        line_31 = grbs.read(1)[0] #read next line, line 31
        line_32 = grbs.read(1)[0] # read next line, line 32
        
        max_up = line_31.values
        ...
        UD = m.contour(HRRR_lon,HRRR_lat,max_up,plot_levels,cmap='bwr',vmin=-2,vmax=2)
        DD = m.contour(HRRR_lon,HRRR_lat,max_down,plot_levels,cmap='bwr',vmin=-2,vmax=2)
        ...
        plt.title('Pioneer Fire\nHRRR Max Up/Down drafts\n %02d hour forecast valid 5 August 2016 %s UTC' % (forecast_h,valid_time)) 
        plt.savefig(SAVEDIR+'PioneerFire_UpDownDraft_'+save_name+'.png',bbox_inches='tight')     
        
        for lc in UD.collections:
            lc.remove()
        for lc in DD.collections:
            lc.remove()
