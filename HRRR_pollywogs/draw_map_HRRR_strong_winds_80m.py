...
for this in range(0,len(dirs)):
    SAVEDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/Pioneer_Fire/HRRR_HighWind/80m/'+dirs[this]+'/'
    ...
    for analysis_h in ah:
        ...
        save_name= 'HRRR_80m_Wind_%02df%02d' % (analysis_h,forecast_h)
        ...
        HRRR_u = grbs.select(name='U component of wind')[-1].values    # m/s
        HRRR_v = grbs.select(name='V component of wind')[-1].values    # m/s
        
        HRRR_speed = wind_calcs.wind_uv_to_spd(HRRR_u,HRRR_v)
        #HRRR_mxspeed = grbs.select(name='80 metre wind speed')[-1].values    # m/s        # 80 metre max wind speed not available 
        ...
        if plotted_cb == False:
            cbar = plt.colorbar(CC, orientation='horizontal',shrink=.8,pad=.04)
