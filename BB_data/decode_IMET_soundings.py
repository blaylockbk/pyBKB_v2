...
## An attempt to decode IMET Sounding Data (these files seem pretty cryptic)
## Copy of the C code can be found on github: 
...
sigHgt2 = barometric_equation_inv(0,sigTemp+273.15,101325,sigPress*100)
  
plt.title(fname+"\n Year: ?  Month:"+month+"  Day:"+day+"  Hour:"+hour+" UTC")
plt.scatter(Temp,Hgt,color="g",label="Manditory")
plt.plot(Temp,Hgt, color="g")

plt.scatter(sigTemp,sigHgt,color="b",label="Significant")
plt.plot(sigTemp,sigHgt,color="b")

plt.scatter(sigTemp,sigHgt2,color="r",label="Significant2")
plt.plot(sigTemp,sigHgt2,color="r")
...
# Now, combine the significant and manditory arrays into one array
newHgt = np.append(Hgt,sigHgt2)
newPres = np.append(Level,sigPress)
newTemp = np.append(Temp,sigTemp)
newDwpt = np.append(Dwpt,sigDwpt)

sortIndexes = np.argsort(newHgt)
sortHgt = np.array(newHgt)[sortIndexes]
sortPres = np.array(newPres)[sortIndexes]
sortTemp = np.array(newTemp)[sortIndexes]
sortDwpt = np.array(newDwpt)[sortIndexes]

plt.plot(sortTemp,sortHgt,'k',linewidth=3,label='combined sig+man')
