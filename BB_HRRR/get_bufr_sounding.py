...
def get_bufr_sounding(request_time, site='kslc', model='hrrr', verbose=False):
    """
    ...
    # 0.  PRES is the pressure in hPa,
    # 1.  TMPC is the temperature in C,
    # 2.  TMWC is the wetbulb in C,
    # 3.  DWPC is the dew point in C,
    # 4.  THTE is theta-e in Kelvin,
    # 5.  DRCT is the wind direction,
    # 6.  SKNT is the wind speed in knots,
    # 7.  OMEG is omega in microbars per second,
    # 0a. CFRL is cloud cover and
    # 1a. HGHT is the height in meters

    p = np.zeros((numhours, numlevs))
    z = np.zeros((numhours, numlevs))
    t = np.zeros((numhours, numlevs))
    tw = np.zeros((numhours, numlevs)) # wet bulb temperature
    td = np.zeros((numhours, numlevs)) # dew point temperature
    wdir = np.zeros((numhours, numlevs))
    wspd = np.zeros((numhours, numlevs))

    thte = np.zeros((numhours, numlevs)) # equivalent potential temperature
    twb = np.zeros((numhours, numlevs))
    ...
    for i in range(numhours):
        ...
        for j in range(len(ind1)):
            ...
            thte[i, j] = float((ls[ind1[j]].rstrip()).split(' ')[4])
        ...
        #   Hack here forces td to be <= t.  Needed due to problems with GFS BUFR data
            ...
            wspd[i, j] = float((ls[ind1[j]].rstrip()).split(' ')[6]) # in knots
    ...
    mydata = dict(zip(('hght', 'pres', 'temp', 'wetb', 'dwpt', 'thte', 'drct', 'sknt', 'fHour', 'DATETIME'), \
                        (z, p, t, tw, td, thte, wdir, wspd, fhr, ptimes)))
