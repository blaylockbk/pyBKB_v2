...
from BB_rose.ozonerose import WindroseAxes
...
def rose_with_labels():    
    ...
    plt.savefig(SAVE + "ozonerose.png")
    #plt.show()

def rose_no_labels():
    """
    There is no grid or labels, just the colorbars.
    I like to overlay these on a map.
    """
    ax = new_axes()
    ax.bar(wd, ws,
           nsector=16,
    ...
    #ax.set_rmax(25)
    ax.set_rmax(np.max(np.sum(ax._info['table'], axis=0)))
    plt.savefig(SAVE + "ozonerose_nolabel.png", transparent=True)
    #plt.show()
...
def clock_rose():
    ...
    plt.savefig(SAVE + "ozone_clock.png")
    #plt.show()
...
if __name__ == '__main__':
    ...
    # Get MesoWest Data
    a = MW.get_mesowest_ts(stn, start, end, variables='wind_direction,ozone_concentration')
