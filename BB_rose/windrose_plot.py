...
def rose_with_labels():
    """
    Create a rose with labels, including grid, tick labels, and title
    """
    ax = new_axes()
    ax.bar(wd, ws,
...
def rose_no_labels():
    ...
    #ax.set_rmax(25)
    ax.set_rmax(np.max(np.sum(ax._info['table'], axis=0)))
    plt.savefig(SAVE + "windros_nolabel.png", transparent=True)
    #plt.show()
...
def clock_rose():
    ...
    plt.savefig(SAVE + "windrose_clock.png")
    #plt.show()
