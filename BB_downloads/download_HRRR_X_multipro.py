...
def get_pressure_field(item):
    ...
    and datetime.strptime(item[0:5], '%y%j').day==yesterday.day:
        ...
        # need to log onto the ftp for each process
        ftp = FTP('gsdftp.fsl.noaa.gov')
        ftp.login(user, password)
        ftp.cwd('hrrr/conus/wrfprs')
        
        # item file name in the form... YYJJJHH00FF00
        #print item, datetime.strptime(item[0:5], '%y%j')
...
if __name__ == '__main__':
    ...
    num_proc = 4
    p = multiprocessing.Pool(num_proc)
    p.map(get_pressure_field, filenames)


    # Print a table of the files that are in the archive
    check_downloads()
